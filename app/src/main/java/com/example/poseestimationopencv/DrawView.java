...
public class DrawView extends View {
     ...
     * @param point 2*14
     */
    public float[][] setDrawPoint(float[][] point, float ratio) {
        mDrawPoint.clear();
        float tempX, tempY;
        float[][] resultPoints = new float[2][14];
        for (int i = 0; i < 14; i++) {
            tempX = point[0][i] / ratio / mRatioX;
            tempY = point[1][i] / ratio / mRatioY;
            resultPoints[0][i] = tempX;
            resultPoints[1][i] = tempY;
